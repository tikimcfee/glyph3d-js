...
/** lane index -> the contract's field name, DERIVED from the S_* exports so the mapping
 *  cannot drift from the lane table it describes. */
const LANE_NAME = {
    [S_GLYPH_ID]: 'GLYPH_ID', [S_ADVANCE]: 'ADVANCE', [S_HEIGHT]: 'HEIGHT',
    [S_X]: 'X', [S_Y]: 'Y', [S_Z]: 'Z', [S_ROW]: 'ROW', [S_COL]: 'COL',
    [S_FLAGS]: 'FLAGS', [S_BASE_X]: 'BASE_X', [S_LINE_ADV]: 'LINE_ADV', [S_ORD]: 'ORD',
};
...
console.log('the render-read lane set is a prefix (the record format depends on it)');
{
    ...
    // glyphContract.js exports VERTEX_READ — but which lanes the render path reads is a
    // fact about glyphVertex.js, and the contract is generated by the backend's tooling.
    // Two sources of truth for one fact is how ITEM_STRIDE drifted to 16 against a live
    // 15 without anything failing. So the contract may STATE it and this side CHECKS it,
    // and a divergence fails here, where the source is.
    {
        const declared = [...VERTEX_READ].sort();
        const extracted = got.map((l) => LANE_NAME[l]).sort();
