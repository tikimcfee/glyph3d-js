...
#
# Env overrides: MULTICA_SRC, MULTICA_PORT, PGPORT, PGDIR, MULTICA_REF
...
DBURL="postgres://multica@/multica?host=${PGSOCK}&sslmode=disable"
...
up() {
  ...
  [ -n "$PGBIN" ] || { echo "multica-up: no postgres server binaries found" >&2; exit 1; }
  ...
  ensure_pg_user
  if ! pg "pg_isready -h '$PGSOCK' -q"; then
    ...
    pg "pg_ctl -D '$PGDIR' -l '$RUNDIR/pg.log' -o \"-k '$PGSOCK'\" start" >/dev/null
    sleep 2
  fi
  pg "createdb -h '$PGSOCK' -U multica multica" 2>/dev/null || true
