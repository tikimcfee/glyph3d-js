...
// A window that records the last border it was told to paint (color/width/strength).
function mockWindow() {
...
// ---- each docked window got a border painted, at the dock's width/strength ----
ok(g1.border && g1.border.strength > 0, 'lock: t1 panel border painted (strength > 0)');
ok(g2.border?.strength > 0 && g3.border?.strength > 0, 'lock: t2 + t3 painted too');
ok(g1.border.width === dock.borderWidth, 'lock: border width = dock.borderWidth');
ok(g1.border.strength === dock.borderStrength, 'lock: border strength = dock.borderStrength');
...
// ---- release clears the border (window leaves the bar → drops its dock identity) ----
dock.spotlight('t2'); // unfocus first (return)
dock.release('t1');
ok(g1.border.strength === 0, 'release: border strength cleared to 0');
