...
out.push('## The rule');
out.push('');
out.push('The slot buffer becomes `uint`. Twelve lanes, three treatments:');
out.push('');
out.push('| lanes | treatment | why |');
out.push('|---|---|---|');
out.push('| `S_ROW` `S_COL` `S_FLAGS` `S_ORD` | **native u32, no bitcast** | genuine counts; exact past 2^24, which is the point of the change |');
...
out.push('| `S_GLYPH_ID` | **DEFERRED** | copied straight from `trie.blocks` (a Float32Array); moving it is a trie/atlas format change, scoped separately |');
out.push('');
