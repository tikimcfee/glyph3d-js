...
// ── Far-texture (the minified text-mass LOD) ─────────────────────────────────
// One shared RGBA8 atlas of fixed-size SLABS, one slab per file (item). Each texel
// pre-integrates the glyphs that land in it: rgb = ink-weighted average syntax color
// (LINEAR — the scatter linearizes before accumulating, so mips average energy, not
// gamma), a = average ink coverage of the texel's grid region. The fragment samples
// it with an explicit mip level in place of the flat impostor block, so minified
// text is a prefiltered, temporally stable content-true mass instead of strobing
// strokes or a content-blind constant. Generation is two dispatches (K10a farScatter,
// K10b farNormalize) that read ONLY previous-dispatch lanes (apply's S_ROW/S_COL,
// decode's S_GLYPH_ID) plus CPU-written lanes (instanceColor via a storage view, the
// gid→ink table, the per-item far lanes) — the pipeline's whole race invariant.

export const FAR_TEX = 1024;          // atlas texels per side
export const FAR_SLAB = 64;           // slab texels per side — one file maps into 64×64
export const FAR_SLABS_PER_SIDE = FAR_TEX / FAR_SLAB;   // 16
export const FAR_MAX_SLABS = FAR_SLABS_PER_SIDE * FAR_SLABS_PER_SIDE;   // 256

/** Per-item far lanes (the kernels' farItems float buffer — ITEM_STRIDE stays full). */
export const FAR_ITEM_STRIDE = 5;
export const FI_SLAB_X = 0;           // slab origin, texel coords (−1 = no slab → impostor)
export const FI_SLAB_Y = 1;
export const FI_ROWS_PER_TEXEL = 2;   // file rows per slab texel row (float; <1 = finer than rows)
export const FI_COLS_PER_TEXEL = 3;   // glyph columns per slab texel column
export const FI_DIRTY = 4;            // 1 = scatter + renormalize at the next far run

export const FAR_FIXED = 1024;        // fixed-point scale of the uint atomic accumulators

/**
 * The CPU ORACLE for K10a (farScatter). Walks every byte exactly as the kernel does —
 * leader+rendered gate, item search, slab/texel mapping, linearize-then-accumulate —
 * and returns the full-atlas uint accumulator. Diff against the GPU's farAccum with a
 ...
 * @param {Object} fns
 * @param {(id:number)=>{r,g,b}} fns.colorAt - per-slot authored (sRGB) color
 * @param {(gid:number)=>number} fns.densityAt - per-glyph ink density, 0..1
 * @returns {Uint32Array} FAR_TEX×FAR_TEX×4 accumulator
 */
export function farScatterOracle(slots, byteLength, items, farItems, { colorAt, densityAt }) {
    const accum = new Uint32Array(FAR_TEX * FAR_TEX * 4);
    for (let id = 0; id < byteLength; id++) {
        const o = id * SLOT_STRIDE;
        const flags = slots[o + S_FLAGS] | 0;
        if ((flags & (F_LEADER | F_RENDERED)) !== (F_LEADER | F_RENDERED)) continue;
        const item = itemForByte(items, id);
        const fb = item * FAR_ITEM_STRIDE;
        const slabX = farItems[fb + FI_SLAB_X] | 0;
        if (slabX < 0 || farItems[fb + FI_DIRTY] < 0.5) continue;
        const slabY = farItems[fb + FI_SLAB_Y] | 0;
        const rpt = farItems[fb + FI_ROWS_PER_TEXEL];
        const cpt = farItems[fb + FI_COLS_PER_TEXEL];
        const row = slots[o + S_ROW], col = slots[o + S_COL];
        ...
        accum[ab]     += Math.trunc(Math.pow(c.r, 2.2) * d * FAR_FIXED);
        accum[ab + 1] += Math.trunc(Math.pow(c.g, 2.2) * d * FAR_FIXED);
        accum[ab + 2] += Math.trunc(Math.pow(c.b, 2.2) * d * FAR_FIXED);
        accum[ab + 3] += Math.trunc(d * FAR_FIXED);
    }
    return accum;
}

/**
 * The CPU ORACLE for K10b (farNormalize): one slab's accumulator → its RGBA8 texels
 * (as packed uint32, the kernel's farPacked layout). rgb = the ink-weighted average
 * (sum c·d / sum d); a = ink mass per GRID cell (sum d / (rowsPerTexel·colsPerTexel)) —
 * empty grid cells (short lines, blank space) correctly dilute coverage. The kernel
 * also zeroes the accumulator after reading (self-cleaning; the next batch starts
 * from zero) — the oracle mirrors that by taking ownership of the passed accum.
 *
 * @param {Uint32Array} accum - full-atlas accumulator (farScatterOracle's output)
 * @param {number} slabX @param {number} slabY - slab origin, texel coords
 * @param {number} rowsPerTexel @param {number} colsPerTexel
 * @returns {Uint32Array} FAR_SLAB² packed RGBA8 texels (r | g<<8 | b<<16 | a<<24)
 */
export function farNormalizeOracle(accum, slabX, slabY, rowsPerTexel, colsPerTexel) {
    const out = new Uint32Array(FAR_SLAB * FAR_SLAB);
    const cell = rowsPerTexel * colsPerTexel * FAR_FIXED;
    const pack = (v) => Math.trunc(Math.min(1, Math.max(0, v)) * 255 + 0.5);
    for (let ty = 0; ty < FAR_SLAB; ty++) {
        for (let tx = 0; tx < FAR_SLAB; tx++) {
            const ab = ((slabY + ty) * FAR_TEX + slabX + tx) * 4;
            const dS = accum[ab + 3];
            const inv = dS > 0 ? 1 / dS : 0;
            const r = accum[ab] * inv, g = accum[ab + 1] * inv, b = accum[ab + 2] * inv;
            const a = dS / cell;
            out[ty * FAR_SLAB + tx] =
                (pack(r) | (pack(g) << 8) | (pack(b) << 16) | (pack(a) << 24)) >>> 0;
            accum[ab] = accum[ab + 1] = accum[ab + 2] = accum[ab + 3] = 0;
        }
    }
    return out;
}
