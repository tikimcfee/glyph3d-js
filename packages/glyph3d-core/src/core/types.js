 ...
 * Generated by GlyphAtlas.generate() or loaded from a pre-baked PNG + JSON.
 * Carries everything a GPU backend needs to reconstruct the atlas binding.
 *
 ...
 * @property {number} atlasMapWidth       - DataTexture width (texels) for the codepoint→UV map
 * @property {number} atlasMapHeight      - DataTexture height (rows) for the codepoint→UV map
 * @property {Object} glyphs             - keyed by grapheme string:
 *   {id: number, u0: number, v0: number, u1: number, v1: number, width: number, height: number, advance: number}
 * @property {Object} glyphWidths        - keyed by grapheme string: pixel width used for layout advance
 * @property {HTMLCanvasElement|null} atlasImage - platform-specific bitmap canvas (null if pre-baked)
 * @property {Float32Array} atlasMapData  - RGBA32F flat array: [u0_webgl, v0_webgl, u1_webgl, v1_webgl]
 *   per glyph slot, laid out row-major matching the DataTexture dimensions
 */
